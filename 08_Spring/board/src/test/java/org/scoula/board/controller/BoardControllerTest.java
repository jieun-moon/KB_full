...
//ServletConfig.class: controller
...
//    mockMvc.perform: 요청을 만드는 과정
//    ResultActions: .andReturn().getModelAndView().getModelMap()
//    list: 키명. addAttribute했을 때 넣었던 명칭으로 거기에 담긴 리스트(모델에 결과가 담겼구나 확인)
    @Test
    public void list() throws Exception {
        log.info(
//              "board/list"을 처리하는 controller의 list()메소드 호출
                mockMvc.perform(MockMvcRequestBuilders.get("/board/list")) //ResultActions 타입 리턴
                        .andReturn() //MvCResult리턴
                        .getModelAndView() //ModelAndView 리턴
                        .getModelMap() //Model 리턴 (BoardController에 있는 Model model 리턴)
        );
    }
    ...
    public void create() throws Exception {
        String resultPage = mockMvc
                .perform(
//                        Body의 내용을 어떻게 채울거냐가 중요
//                        title: 키, title의 실제 값이 값으로 들어감
                ...
                .andReturn()
                .getModelAndView()
                .getViewName(); //getViewName: redirectURL 추출 => String으로 받아왔으므로
        //해당하는 뷰의 이름 return
