-- 다음 결과가 나오도록 buytbl 에 대한 SQL 문을 각각 작성하세요
USE sqldb;
SELECT * FROM buytbl;
...
-- name, height
-- MAX로는 가장 키가 큰 사람, MIN으로는 가장 키가 작은 사람 출력 가능
...
-- WHERE 절은 그룹화하기 전에 필터링하는 작업이다
-- 따라서 그룹화된 데이터에 조건을 추가하려면 HAVING을 사용해야 한다
SELECT userID AS 사용자, SUM(price*amount) AS '총 구매액'
FROM buytbl
GROUP BY userID
HAVING 1000 <= SUM(price*amount);

-- world 데이터베이스에서 다음 질문을 처리하세요
USE world;

-- city 테이블에서 국가코드가 'KOR'인 도시들의 인구수 총합을 구하시오
SELECT SUM(population) AS '한국 인구수 총합' FROM city
...
-- ROUND: 반올림(반올림할 값, 몇 째 자리에서 반올림할지)
