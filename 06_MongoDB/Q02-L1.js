...
//users 컬렉션에 username 이 smith 인 문서를 저장하세요
//db는 현재 사용중인 데이터베이스를 설정한 전영 변수
//insert(): 문서 삽입
db.users.insert({username: "smith"});

//users 컬렉션에 username 이 jones 인 문서를 저장하세요
...
//$or 조건 검사: 객체 배열로 여러 개의 조건을 묶어준다
//두 가지 조건 중 하나만 만족해도 출력된다
db.users.find({$or:[
    {username:"smith"},
    {username:"jones"}
    ]});

//users 컬렉션에서 username 이 smith 인 문서에 country 키가 Canada 가 되도록 수정하세요
//update(찾을 문서의 조건, 변경할 문서)
//이외의 파라미터는 생략 가능, 생략하면 defaulst 값이 들어간다
...
//users 컬렉션에서 username 이 smith 인 문서를 {country: "Canada"}로 대체하고 확인하세요
//$set이 없으면 문서 전체를 변경할 문서로 덮어씌운다
//MongoInvalidArgumentError: 업데이트 연산자 필요
db.users.update({username: "smith"}, {country: "Canada"});

//users 컬렉션에서 username 이 smith 인 문서를 {username: "smith", country: "Canada"} 로 대체하고 확인하세요
//MongoInvalidArgumentError: 업데이트 연산자 필요
...
//users 컬렉션에서 username 이 smith 인 문서를 삭제하고 , 삭제여부를 확인하세요
//remove(): 조건에 해당하는 문서를 삭제
//기본적으로 조건에 해당하는 문서를 모두 삭제해줌
